...
-- --------------------------------------------------
-- Date Created: 12/15/2016 22:05:14
...
GO
IF OBJECT_ID(N'[JongHorDBModel1StoreContainer].[FK_Dorm_Option_Dorm]', 'F') IS NOT NULL
    ALTER TABLE [JongHorDBModel1StoreContainer].[Dorm_Option] DROP CONSTRAINT [FK_Dorm_Option_Dorm];
GO
...
GO
IF OBJECT_ID(N'[dbo].[FK_Person_Dorm]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Person] DROP CONSTRAINT [FK_Person_Dorm];
GO
IF OBJECT_ID(N'[dbo].[FK_Person_Room]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Person] DROP CONSTRAINT [FK_Person_Room];
GO
IF OBJECT_ID(N'[JongHorDBModel1StoreContainer].[FK_Room_Option_Room_Type]', 'F') IS NOT NULL
    ALTER TABLE [JongHorDBModel1StoreContainer].[Room_Option] DROP CONSTRAINT [FK_Room_Option_Room_Type];
GO
...
GO
IF OBJECT_ID(N'[dbo].[FK_Room_ToDorm]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Room] DROP CONSTRAINT [FK_Room_ToDorm];
GO
...
-- Creating primary key on [Reserve_ID] in table 'Room_Reserved'
...
-- Creating foreign key on [Dorm_ID] in table 'Person'
ALTER TABLE [dbo].[Person]
ADD CONSTRAINT [FK_Person_Dorm]
    FOREIGN KEY ([Dorm_ID])
    REFERENCES [dbo].[Dorm]
        ([Dorm_ID])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_Person_Dorm'
CREATE INDEX [IX_FK_Person_Dorm]
ON [dbo].[Person]
    ([Dorm_ID]);
GO
...
-- Creating non-clustered index for FOREIGN KEY 'FK_Message_Person'
CREATE INDEX [IX_FK_Message_Person]
ON [dbo].[Message]
    ([Sender_Username]);
GO
...
-- Creating non-clustered index for FOREIGN KEY 'FK_Room_Reserved_Person'
CREATE INDEX [IX_FK_Room_Reserved_Person]
...
-- Creating foreign key on [Dorm_ID] in table 'Room'
ALTER TABLE [dbo].[Room]
ADD CONSTRAINT [FK_Room_ToDorm]
    FOREIGN KEY ([Dorm_ID])
    REFERENCES [dbo].[Dorm]
        ([Dorm_ID])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_Room_ToDorm'
CREATE INDEX [IX_FK_Room_ToDorm]
ON [dbo].[Room]
    ([Dorm_ID]);
GO
